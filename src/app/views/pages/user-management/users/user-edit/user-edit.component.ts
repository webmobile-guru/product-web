...
export class UserEditComponent implements OnInit, OnDestroy {
	...
	ngOnInit() {
		...
		const routeSubscription = this.activatedRoute.params.subscribe(params => {
			...
			if (id && (id > 0 || id != '')) {
				console.log('to edit user data id: ', id)
				this.store.pipe(select(selectUserById(id))).subscribe(res => {
					if (res) {
						console.log('go to edit user data: ', res)
						this.user = res;
				...
				console.log('go to create new user data: ', id)
	 ...
	 * @param id: number
	 */
	refreshUser(isNew: boolean = false, id = undefined) {
	...
	onSumbit(withBack: boolean = false) {
		...
		if (editedUser.id) {
			console.log('we go to edit user')
		// if (editedUser.id > 0) {
			this.updateUser(editedUser, withBack);
			return;
		}
		console.log('we go to create new user')
	...
	updateUser(_user: User, withBack: boolean = false) {
		...
		console.log('gonna update user data; ', updatedUser, _user)
